...
export async function POST(request: NextRequest) {
  try {
    ...
    if (submission.gradingResult) {
      logger.info({ submissionId }, 'Submission already graded, skipping')
      return NextResponse.json({ 
        success: true, 
    ...
    if (gradingResult) {
      ...
      }, 'Background grading completed successfully')
      ...
      logger.error({ submissionId }, 'Background grading failed')
      return NextResponse.json(
